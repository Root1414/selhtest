...
### 7. N8N_EXECUTION_LOG (سجل تنفيذ n8n) ⭐ NEW

**Purpose:** Tracks every n8n workflow execution for audit, debugging, and performance monitoring

| Column | Type | Nullable | Description |
|--------|------|----------|-------------|
| `id` | INTEGER | NO | Primary key, auto-increment |
| `workflow_name` | VARCHAR(100) | NO | e.g., "ai-match-workflow", "compliance-monitor" |
| `execution_id` | VARCHAR(100) | YES | n8n's internal execution ID |
| `status` | ENUM('success', 'error') | NO | Execution outcome |
| `input_payload` | TEXT | YES | JSON input sent to n8n |
| `output_summary` | TEXT | YES | JSON summary of n8n response |
| `error_message` | TEXT | YES | Error details if failed |
| `duration_ms` | INTEGER | YES | Execution time in milliseconds |
| `executed_at` | DATETIME | NO | Timestamp of execution |
...
### Phase 2 → Phase 3 (n8n Vector Store)
- Keep SQL for structured data
- Use n8n's built-in **Vector Store** node (replaces JS cosine similarity code)
- Sync LOCAL_PRODUCT descriptions → vector embeddings on insert/update
- n8n AI Match Workflow queries vector store directly, then joins with SQL for metadata
